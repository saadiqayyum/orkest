...
async def identify_new_post_urls(
    ...
    """
    Identify which URLs need content fetching.

    This includes:
    1. URLs not yet in the posts table (truly new)
    2. URLs that exist but need content re-fetch:
       - Post is paywalled (is_paywalled=true)
       - Post doesn't have full content (has_full_content=false)
       - Publication is in PAID_SUBSCRIPTION_HANDLES (we have access)
    ...
    - new_posts: Full post objects for URLs that need fetching (new + needs refetch)
    """
    urls = params.urls or []
    ...
    # Get paid subscription handles from secrets (stored as JSON array in vault)
    paid_handles_raw = ctx.secrets.get("PAID_SUBSCRIPTION_HANDLES") if ctx.secrets else None
    paid_handles = set(paid_handles_raw) if paid_handles_raw else set()

    ctx.report_input({
        "urls_count": len(urls),
        "posts_count": len(posts),
        "paid_handles": list(paid_handles),
    })
    ...
    async with get_db_session() as db:
        ...
        # Find posts that need content re-fetch:
        # - Exist in DB, paywalled, no full content, and publication is in paid handles
        # - Exclude 'founding' tier posts (we can't access those even with paid subscription)
        needs_refetch_urls = []
        if paid_handles and existing_urls:
            result = await db.execute(
                select(Post.url).where(
                    and_(
                        Post.url.in_(existing_urls),
                        Post.is_paywalled == True,
                        or_(Post.has_full_content == False, Post.has_full_content.is_(None)),
                        Post.publication_handle.in_(paid_handles),
                        # Skip founding tier - requires higher subscription we don't have
                        or_(Post.audience != 'founding', Post.audience.is_(None)),
                    )
                )
            )
            needs_refetch_urls = [row[0] for row in result.fetchall()]

        # Combine new URLs and URLs needing refetch
        all_urls_to_fetch = list(set(new_urls + needs_refetch_urls))
        new_posts = [posts_by_url[url] for url in all_urls_to_fetch if url in posts_by_url]

        logger.info(
            "new_post_urls_identified",
            total=len(urls),
            new=len(new_urls),
            needs_refetch=len(needs_refetch_urls),
            existing=len(existing_urls),
            paid_handles=list(paid_handles),
        )

        # Determine status - use no_new_items if nothing to fetch
        status = "no_new_items" if not all_urls_to_fetch else "success"

        ctx.report_output({
            "new_urls_count": len(new_urls),
            "needs_refetch_count": len(needs_refetch_urls),
            "existing_urls_count": len(existing_urls),
            "sample_new_urls": new_urls[:5],
            "sample_refetch_urls": needs_refetch_urls[:5],
            "status": status,
        })

        return IdentifyNewPostUrlsOutput(
            new_urls=all_urls_to_fetch,  # Return all URLs that need fetching
