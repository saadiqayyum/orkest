...
def _get_llm_config(ctx, llm_config: Optional[LLMConfig] = None) -> dict:
    ...
    0. LLM_OVERRIDE_PROVIDER env var - Force override (for dev/testing)
    ...
    All config comes from os.environ (project_env gets injected there at workflow start).

    Args:
        ctx: Execution context (unused, kept for API compatibility)
        llm_config: Optional override from node params (can be dict or LLMConfig)

    Returns:
        dict with provider, model, temperature, and API keys
    """
    # 0. Force override - ignores all other config (useful for local dev)
    override_provider = os.environ.get("LLM_OVERRIDE_PROVIDER")
    if override_provider:
        override_model = os.environ.get("LLM_OVERRIDE_MODEL") or os.environ.get("LLM_MODEL") or "llama3.1"
        # Still respect temperature from node params if provided
        temperature = None
        if llm_config:
...
def _get_image_config(ctx) -> dict:
    """
    Get image generation configuration from os.environ (project_env injected at workflow start).
    """
    return {
        "provider": os.environ.get("IMAGE_PROVIDER") or "placeholder",  # placeholder, openai, stable-diffusion, gemini
        "image_model": os.environ.get("IMAGE_MODEL") or "gemini-3-pro-image-preview",
        "openai_api_key": os.environ.get("OPENAI_API_KEY"),
        "google_api_key": os.environ.get("GOOGLE_API_KEY"),
        "sd_api_url": os.environ.get("SD_API_URL") or "http://localhost:7860",  # Automatic1111/ComfyUI
    }
