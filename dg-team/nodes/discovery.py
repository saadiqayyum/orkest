...
def _normalize_keywords_input(keywords) -> str:
    """
    Normalize keywords input to semicolon-separated string.
    Handles both array and string inputs for backwards compatibility.

    Array input: ["keyword1", "keyword2"] -> "keyword1; keyword2"
    String input: "keyword1, keyword2" -> "keyword1; keyword2"
    """
    if keywords is None:
        return ""
    if isinstance(keywords, list):
        return "; ".join(str(k).strip() for k in keywords if k)
    # Already a string - normalize commas to semicolons
...
async def score_keyword_relevance(
    ...
    """
    publications = _ensure_publications(_get_param(params, "publications"))
    keywords_str = _normalize_keywords_input(_get_param(params, "keywords"))
...
async def score_llm_relevance(
    ...
    publications = _ensure_publications(_get_param(params, "publications"))
    keywords = _normalize_keywords_input(_get_param(params, "keywords"))
