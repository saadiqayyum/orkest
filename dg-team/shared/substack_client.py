...
Caching strategy:
- Post listings (fetch_post_index, fetch_all_posts): Cached for 24 hours - most publications don't post daily
...
# Rate limiting - Substack returns 429 if too fast
REQUEST_DELAY = 1.0  # 1s between requests (was 500ms, still caused 429s)


class FetchStatus:
    """Status codes for fetch operations."""
    SUCCESS = "success"          # Content fetched successfully
    NOT_FOUND = "not_found"      # 404 - post no longer exists
    RATE_LIMITED = "rate_limited"  # 429 - too many requests
    AUTH_REQUIRED = "auth_required"  # 401/403 - needs authentication
...
class FetchResult:
    """
    Result of a fetch operation with explicit status.

    This provides a clean API where callers can check the status
    without relying on exception handling for expected cases like 404s.
    """
    status: str  # One of FetchStatus values
    post: Optional['SubstackPost'] = None  # The fetched post (if successful)
    from_cache: bool = False  # Whether content came from cache
    ...
    def is_not_found(self) -> bool:
        return self.status == FetchStatus.NOT_FOUND

    @property
    def is_retriable(self) -> bool:
        """Returns True if this error is transient and worth retrying."""
        return self.status in (FetchStatus.RATE_LIMITED, FetchStatus.ERROR)
...
class SubstackClient:
    ...
    ) -> List[SubstackPost] | Tuple[List[SubstackPost], bool]:
        """
        Fetch post listings (metadata only) from a publication.

        Cached for 24 hours - most publications don't post more than once a day.

        Args:
            handle: Substack publication handle
            sort: Sort order - 'new' or 'top'
            limit: Maximum posts to fetch
            return_cache_status: If True, returns (posts, from_cache) tuple

        Returns:
            List of SubstackPost with metadata (no content), or (posts, from_cache) tuple
        """
        base_url = f"https://{handle}.substack.com"
        api_url = f"{base_url}/api/v1/archive?sort={sort}&limit={limit}"

        try:
            content, from_cache = await self._fetch_with_cache(api_url, cache_ttl_hours=24)
            ...
            logger.info("post_index_fetched", handle=handle, sort=sort, count=len(posts), from_cache=from_cache)

            if return_cache_status:
                return posts, from_cache
            return posts
    ...
    async def fetch_all_posts(
        ...
        page_size: int = 12,
        return_cache_status: bool = False,
    ) -> List[SubstackPost] | Tuple[List[SubstackPost], int, int]:
        """
        Fetch all posts from a publication (paginated).

        Cached for 24 hours per page - most publications don't post more than once a day.

        Args:
            handle: Substack publication handle
            max_pages: Maximum pages to fetch (safety limit)
            page_size: Posts per page
            return_cache_status: If True, returns (posts, cache_hits, total_pages) tuple

        Returns:
            List of all SubstackPost with metadata, or (posts, cache_hits, total_pages) tuple
        """
        base_url = f"https://{handle}.substack.com"
        posts = []
        offset = 0
        ...
        try:
            for _ in range(max_pages):
                ...
                offset += page_size
                # Only delay if we didn't hit the cache
                if not from_cache:
                    await asyncio.sleep(REQUEST_DELAY)

            logger.info("all_posts_fetched", handle=handle, count=len(posts), cache_hits=cache_hits, total_pages=total_pages)

            if return_cache_status:
                return posts, cache_hits, total_pages
            return posts
    ...
    async def fetch_post_content(
        ...
        force_refresh: bool = False,
        return_cache_status: bool = False,
    ) -> Optional[SubstackPost] | Tuple[Optional[SubstackPost], bool]:
        """
        ...
        Args:
            ...
            return_cache_status: If True, returns (post, from_cache) tuple

        Returns:
            SubstackPost with content_raw populated, or None on failure.
            If return_cache_status=True, returns (SubstackPost, from_cache) tuple.
        """
        try:
            ...
            if return_cache_status:
                return post, from_cache
            return post

        except httpx.HTTPStatusError as e:
            # Re-raise HTTP errors so caller can handle 404s etc specially
    ...
    async def fetch_post_content_with_status(
        self,
        url: str,
        force_refresh: bool = False,
    ) -> FetchResult:
        """
        Fetch full content for a single post with explicit status handling.

        This is the preferred API - it returns a FetchResult that explicitly
        indicates success, not found, auth required, etc. instead of using
        ...
        Returns:
            FetchResult with status, post (if successful), and error info
        """
        try:
            content, from_cache = await self._fetch_with_cache(
                url,
            ...
            logger.warning("post_content_http_error", url=url, status=status_code)
            ...
            if status_code == 404:
                ...
                return FetchResult(
                    ...
                    error=f"Authentication required ({status_code})",
